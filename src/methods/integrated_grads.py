...
class IntegratedGradientsExplainer:
    def __init__(self, model, tokenizer):
        ...
        self.pipe = TextClassificationPipeline(model=model, tokenizer=tokenizer, return_all_scores=True, truncation = True, max_length = 512, device = 0)

        self.explainer = SequenceClassificationExplainer(model, tokenizer)

    def _predict_proba(self, sentence):
        probas = []
        # print(self.pipe(sentence))
        for label in self.pipe(sentence)[0]:
